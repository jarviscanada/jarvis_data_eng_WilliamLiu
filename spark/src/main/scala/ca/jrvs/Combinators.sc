...
val ls_last = ls.last
val ls_first_5 = ls.take(5)
val ls_last_5 = ls.takeRight(5)
...
numList.map(i => i.map(_ * 2)).flatten
 ...
 */
//write you solution here
names.map(name => countryMap.getOrElse(name, "n/a")).groupBy(country => country).map {
  case (country, ls) => (country, ls.size)
}
...
//write you solution here
names2.map(name => (name, names2.indexOf(name) + 1))
 ...
 */
//write you solution here
val lines = io.Source.fromFile("/home/centos/dev/jarvis_data_eng_WilliamLiu/spark/src/main/resources/employees.csv").getLines().toList
...
class Employee(var id: Int, var name: String, var city: String) {
  override def toString: String = s"Employee($id,$name,$city)"
}

val employees = lines.drop(1).map(line => {
  val split = line.split(",")
  new Employee(split(0).toInt, split(1), split(2))
})
 ...
 */
//write you solution here
class EmployeeWithAge(var id: Int, var name: String, var city: String, var age: Int) {
  override def toString: String = s"Employee($id,$name,$city,$age)"
}

val upperCity = lines.drop(1).map(line => {
  val split = line.split(",")
  new EmployeeWithAge(split(0).toInt, split(1), split(2).toUpperCase(), split(3).toInt)
})
 ...
 */
//write you solution here
val cityNum = lines.drop(1).map(line => {
  val split = line.split(",")
  new EmployeeWithAge(split(0).toInt, split(1), split(2).toUpperCase(), split(3).toInt)
}).groupBy(employee => employee.city).map { case (city, ls) => (city, ls.size) }
...
lines.drop(1).map(line => {
  val split = line.split(",")
  new EmployeeWithAge(split(0).toInt, split(1), split(2).toUpperCase(), split(3).toInt)
}).groupBy(employee => (employee.city, employee.age)).map { case (city, ls) => (city, ls.size) }
